...
const Portfolio = () => {
  const [filterFocusId, setFilterFocusId] = useState<string | null | undefined>(content.portfolio.filters[0].id);
  const [filterHoverId, setFilterHoverId] = useState<string | null | undefined>();
  const [filteredArtworks, setFilteredArtworks] = useState<Artwork[]>(data);
  const [artworks, setArtworks] = useState<Columns>(splitToColumns(filteredArtworks));
  const [artworkHoverId, setArtworkHoverId] = useState<number | null | undefined>();

  useEffect(() => {
    setArtworks(splitToColumns(filteredArtworks));
  }, [filteredArtworks])

  const { screenS, screenM, screenL } = useScreenSize();
    ...
    <Section title={content.portfolio.title}>      
      <ul style={filtersCSS}>
        {content.portfolio.filters.map(({id, text}) => (
          <li
            ...
            onMouseOut={() => setFilterHoverId(isFilterFocused(id) ? filterHoverId : null)}
      ...
      >
        {Object.entries(artworks).map(([key, value]) => {
          return (
            <div key={key}
              style={columnCSS(screenS, screenL)}
            >
              {value.map(({
                id,
                name,
